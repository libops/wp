...
Dockerized deployment of a Composer-managed [Bedrock](https://roots.io/bedrock/) WordPress project based on the [Islandora Buildkit](https://github.com/Islandora-Devops/isle-buildkit) WordPress PHP/nginx base image.
...
Traefik is always part of the stack and is the only published ingress. The `wp` container listens only on the internal Compose network, while Traefik publishes `${HOST_INSECURE_PORT:-80}` and routes requests to WordPress.

`docker-compose.yaml` is the production-shaped default. Local development changes should be copied from `docker-compose.override-example.yaml` to `docker-compose.override.yaml`; the example only adds local debugging conveniences and does not replace the production ingress path.
...
If you want live host edits in local development, copy `docker-compose.override-example.yaml` to `docker-compose.override.yaml`. That override mounts `web/app/plugins` and `web/app/themes` into `wp`.
...
PHP `mail()` in the `wp` container is wired through `msmtp`, which relays by default to `${LIBOPS_SMTP_HOST:-host.docker.internal}:${LIBOPS_SMTP_PORT:-25}`. That makes the running site use the Docker host's mail server without needing a WordPress SMTP plugin for the common local/dev case.
...
- `docker compose build --pull` installs Composer dependencies into the image.
- `docker compose run --rm init` generates secrets for the project.
...
- The image also includes `msmtp`; PHP `mail()` relays to `${LIBOPS_SMTP_HOST:-host.docker.internal}:${LIBOPS_SMTP_PORT:-25}` by default.
...
The compose stack includes Traefik for HTTP routing. Run the stack with the base compose file:
...
The base compose stack publishes port `80`. The repo keeps Traefik dynamic config in `./conf/traefik/wordpress.tmpl`, which Compose mounts into the container as a file-provider `.yml` config.
